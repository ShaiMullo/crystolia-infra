...
variable "go_live" {
  description = <<-EOT
    ...
    crystolia.co.il at CloudFront (makes the domain live).

    Two-apply sequence (same as crystolia.ru):
      1. apply with go_live=false  -> creates S3 + ACM + CloudFront (no apex/www
         DNS; ACM DNS-validation CNAMEs are created so the cert can issue).
